...
5. Set the GitHub repository variables `BLOG_ENGINE_REPOSITORY`, `BLOG_ENGINE_COMMIT`, and `CLOUDFLARE_PROJECT_NAME`. Derive the engine repository and commit from this repository's Git remote and current commit. Verify that the engine is reachable from GitHub Actions.
