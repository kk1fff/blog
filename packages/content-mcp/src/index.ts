...
server.tool("set_site_settings", "Set managed site metadata, author profile, built-in theme, and public Giscus comments configuration.", { title: z.string().optional(), description: z.string().optional(), language: z.string().optional(), author: z.object({ greeting: z.string().min(1), bio: z.string().min(1), links: z.object({ github: z.string().url().optional(), linkedin: z.string().url().optional() }).optional() }).optional(), theme: z.enum(["paper", "journal", "mono"]).optional(), giscus: z.object({ repo: z.string().regex(/^[^/\\s]+\/[^/\\s]+$/), repoId: z.string().min(1), category: z.string().min(1), categoryId: z.string().min(1) }).optional() }, async (input) => {
  const settings = await readSettings(dir); settings.site = { ...settings.site, ...(input.title ? { title: input.title } : {}), ...(input.description ? { description: input.description } : {}), ...(input.language ? { language: input.language } : {}), ...(input.author ? { author: input.author } : {}) }; if (input.theme) settings.theme = input.theme; if (input.giscus) settings.comments = { giscus: input.giscus }; await writeSettings(settings, dir); return text(settings);
});
